...
// DOM元素 - 使用现有的pages变量，不重复声明
const adminPages = document.querySelectorAll('.admin-page');

// 页面切换函数 - 使用现有的showPage函数，不重复声明

// 显示最新公告
async function showLatestAnnouncements() {
    const announcementModal = document.getElementById('announcement-modal');
    const announcementContent = document.getElementById('announcement-content');
    
    try {
        // 从Supabase获取公告
        const announcements = await window.supabaseService.announcements.getAll();
...
async function confirmBorrow() {
    ...
    try {
        // 查找或创建学生
        let student = await window.supabaseService.readers.getById(studentId);
        
        if (!student) {
            // 新学生注册
            student = await window.supabaseService.readers.add({
                id: studentId,
                name: studentName
            });
        }
        ...
        if (bookIsbn) {
            // 从Supabase根据ISBN查找图书
            const books = await window.supabaseService.books.getAll();
        ...
        // 检查借阅数量限制
        const borrowRecords = await window.supabaseService.borrowRecords.getAll();
        ...
        // 执行借阅
        const updatedBook = await window.supabaseService.books.update(book.id, {
            available: book.available - 1,
            status: book.available === 1 ? 'out' : book.status
        });
        ...
        // 创建借阅记录
        await window.supabaseService.borrowRecords.add({
            studentId: studentId,
...
async function initBookList() {
    ...
    try {
        ...
        if (pageActions && !document.getElementById('export-books-btn')) {
            const exportBtn = document.createElement('button');
            exportBtn.id = 'export-books-btn';
            exportBtn.className = 'btn btn-secondary';
            exportBtn.innerHTML = '<i class="fas fa-download"></i> 导出图书';
            exportBtn.onclick = () => exportToCSV(books, '图书信息.csv');
        ...
        bookList.innerHTML = '<p>获取图书列表失败，请刷新页面重试</p>';
    }
}
...
function confirmEditBook() {
    ...
    if (title && author && isbn && category && publisher && !isNaN(copies)) {
        ...
        book.copies = copies;
        book.available = Math.min(book.available, copies);
...
// 读者管理功能
async function initReaderTree() {
    const readerTree = document.getElementById('reader-tree');
    const readerDetails = document.getElementById('reader-details');
    
    try {
        // 从Supabase获取读者和借阅记录数据
        const readers = await window.supabaseService.readers.getAll();
        const borrowRecords = await window.supabaseService.borrowRecords.getAll();
        
        // 添加导出按钮
        const readersPage = document.getElementById('readers');
        const pageHeader = readersPage.querySelector('.page-header');
        if (pageHeader && !document.getElementById('export-readers-btn')) {
            const exportBtn = document.createElement('button');
            exportBtn.id = 'export-readers-btn';
            exportBtn.className = 'btn btn-primary';
            exportBtn.innerHTML = '<i class="fas fa-download"></i> 导出读者';
            exportBtn.onclick = () => exportToCSV(readers, '读者信息.csv');
            pageHeader.appendChild(exportBtn);
        }
        
        // 按学号分组（简化，只显示学号和姓名）
        readerTree.innerHTML = readers.map(reader => `
            <div class="reader-item">
                <div class="student-item" onclick="showReaderDetails('${reader.id}')">
        ...
        // 默认显示第一个读者的详细信息
        if (readers.length > 0) {
            showReaderDetails(readers[0].id, readers, borrowRecords);
        }
    } catch (error) {
        console.error('获取读者列表失败:', error);
        readerTree.innerHTML = '<p>获取读者列表失败，请刷新页面重试</p>';
    }
}

function showReaderDetails(studentId, readers = null, borrowRecords = null) {
    // 如果没有传入数据，从Supabase获取
    if (!readers || !borrowRecords) {
        window.supabaseService.readers.getAll().then(fetchedReaders => {
            window.supabaseService.borrowRecords.getAll().then(fetchedBorrowRecords => {
                showReaderDetails(studentId, fetchedReaders, fetchedBorrowRecords);
            });
        });
        return;
    }
...
// 借阅记录管理
async function initRecordsList() {
    ...
    try {
        ...
        const pageHeader = recordsPage.querySelector('.page-header');
        const pageActions = pageHeader.querySelector('.page-actions');
        if (pageActions && !document.getElementById('export-records-btn')) {
            const exportBtn = document.createElement('button');
            exportBtn.id = 'export-records-btn';
            exportBtn.className = 'btn btn-secondary';
            exportBtn.innerHTML = '<i class="fas fa-download"></i> 导出借阅记录';
            exportBtn.onclick = () => exportToCSV(borrowRecords, '借阅记录.csv');
            pageActions.appendChild(exportBtn);
        }
        ...
        recordsList.innerHTML = '<p>获取借阅记录失败，请刷新页面重试</p>';
    }
}
...
async function searchISBN() {
    ...
    try {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'user',
                        content: `请提供ISBN ${isbn} 对应的图书信息，包括书名、作者、分类、出版社，格式为JSON：{"title": "", "author": "", "category": "", "publisher": ""}`
                    }
                ],
                temperature: 0.5
            })
        ...
        const bookInfo = JSON.parse(data.choices[0].message.content);
        ...
        alert('ISBN搜索失败，请检查ISBN号码是否正确或稍后重试');
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof initMainSystem === 'function') {
        ...
        // 如果主系统尚未初始化，延迟初始化图书馆系统
        setTimeout(initLibrarySystem, 100);
    }
});
